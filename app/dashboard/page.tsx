...
import { useState, useEffect, forwardRef, Suspense, useRef } from "react";
...
const Input = forwardRef<HTMLInputElement, any>(
	({ className, type, label, ...props }: any, ref) => {
		return (
			<input
				type={type}
				className='flex h-10 w-full rounded-md border border-input bg-white px-3 py-2 text-[14px] ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring disabled:opacity-50'
				ref={ref}
				{...props}
			/>
		);
...
>(({ className, variant, size, ...props }, ref) => {
	return (
		<button
			className='inline-flex items-center justify-center rounded-md text-[14px] font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 h-10 px-4 py-2'
			ref={ref}
			{...props}
		/>
	);
...
const SOCKET_URL = "https://backend.cauhec.org/";
...
const generate_UUID = () => Math.random().toString(36).substr(2, 9);
const calculate_TimeSince = (timestamp: string) => {
...
function DashboardContent() {
	...
	const [auth, setAuth] = useState<any>(
		typeof window !== "undefined"
			? JSON.parse(localStorage.getItem("chat_auth") || "null")
			: null
	);
	...
	const [uploadProgress, set_Upload_Progress] = useState(0);
	const [showFileUpload, setShowFileUpload] = useState(false);
	const [profile_Edit_Mode, setProfileEditMode] = useState(false);
	const [temp_Profile_Data, setTempProfileData] = useState<any>({});
	const [SOCKET_CONNECTED, SET_SOCKET_STATUS] = useState(false);
	const [draft_Messages, setDraftMessages] = useState<any>({});
	const [selectedFile, set_Selected_File] = useState<any>(null);
	...
	const fetchUsers = async () => {
		...
		try {
			const res = await axios.get(
				`${API_URL}/relation?type=accepted&page=1&limit=50`,
				{
			...
			const mapped = raw
				.map((c: any) =>
					c.student?.id == auth?.user?.id ? c.preceptor : c.student
				)
				.filter(Boolean);
			setUsers(mapped);
		} catch (err) {
	...
	useEffect(() => {
		if (search_query) searchUsers(search_query);
	}, [search_query]);

	const searchUsers = async (q: string) => {
		console.log("Searching API with query:", q);
		await axios.get(
			`${API_URL}/relation?type=accepted&name=${q}&page=1&limit=50`,
			{
				headers: { Authorization: `Bearer ${auth?.token}` },
			}
		);
	...
	const loadMessages = async (userId: string) => {
		...
		const response = await axios.get(
			`${API_URL}/chats/${userId}?page=1&pageSize=200`,
			{
				headers: { Authorization: `Bearer ${auth?.token}` },
			}
		);
	...
	const handleSendMessage = () => {
		...
		setNewMessage("");
		setDraftMessages((d: any) => ({ ...d, [SelectedUser.id]: "" }));
	};
	...
	const initSocket = () => {
		...
		newSocket.on("connect", () => {
			...
			newSocket.emit("login", auth.user.id);
		});
				...
				{activeTab === "chat" && !sidebarCollapsed && (
					<div className='p-4 flex-1 overflow-hidden flex flex-col'>
						...
						<div className='flex-1 overflow-y-auto space-y-1'>
							{users
								.filter((u) =>
									u.fullName.toLowerCase().includes(search_query.toLowerCase())
								)
								.map((u) => (
									<button
										key={u.id}
							...
							<div className='bg-white rounded-3xl border overflow-hidden'>
								<table className='w-full'>
									<thead className='bg-gray-50 border-b'>
										<tr>
											<th className='p-4 text-left text-[12px] font-bold uppercase text-gray-400'>
												Session
											</th>
											<th className='p-4 text-left text-[12px] font-bold uppercase text-gray-400'>
												Type
